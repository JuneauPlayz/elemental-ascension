...
@export var lock_horizontal_axis := false # Inspector toggle for horizontal lock
...
func _draw():
	var used_drop_color := drop_color if (_affected_by_free_container == null || !_affected_by_free_container.drop_color_override_children) else _affected_by_free_container.drop_color
	if _mouse_dragging:
...
func get_rect_after_drop() -> Rect2:
	...
	if lock_horizontal_axis:
		result_position.x = locked_x
...
func _get_resize_minimum_size() -> Vector2:
	...
	return Vector2(maxf(result_size.x, custom_minimum_size.x), maxf(result_size.y, custom_minimum_size.y))
